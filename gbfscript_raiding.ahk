; Monitors the clipboard for raid codes and fights them. Pretty experimental at this point.
; Use some python script to dump codes to the clipboard
...
global questUnclaimed := "http://game.granbluefantasy.jp/#quest/assist/unclaimed"
global searchURL := "http://game.granbluefantasy.jp/#quest/assist"
global selectOne := "pending_battle.png"
global selectTwo := "viramate_icon.png"
global join_raid := "viramate_join_raid.png"
global drop_down := "viramate_recover.png"
global selectOne_X := -4
global selectOne_Y := 3
global selectTwo_X := 0
global selectTwo_Y := 0

global genericActions := [selectOne, selectTwo, drop_down, join_raid]
...
main_loop(Type)
{
	if (Type != 1) 
	{
		updateLog("Unknown type of content in clipboard")
		return
	}

	if (RegExMatch(Clipboard, "[A-Z0-9]{8}") = 0 )
	{
		updateLog("Invalid code")
		return
	}
	updateLog("happening")
	...
	Loop
	{
		...
		If (sURL != "")
		{
			if InStr(sURL, questUnclaimed)
			{
				updateLog("-----In Pending-----")
				RandomClick(167, 296, clickVariance)
				Sleep, % default_interval
				Continue
			}
			...
			else if InStr(sURL, searchBattle)
			{
				...
				if InStr(searchResult, attack_button)
				{
					if (attackTurns = 0)
					{
						...
						battleNonActions := 0
						SetOugi(True) ;True for ougi False for nah
						;UsePot(int) ;0 for blue, 1-4 for green pots on characters

						;ClickSummon(int)

						ClickSkill([12,13]) ;ClickSkill now takes a 2/3 digit integer, or an array of them!
			...
			else if InStr(sURL, searchURL)
			{
				...
				{
					updateLog("Pending battle button detected, clicking")
					waitCount := 0
				...
				{
					updateLog("Clicking viramate icon")
					waitCount := 0
				...
				{ 
					updateLog("Not Enough BP dialog found, clicking Use button")
					waitCount := 0
				...
				else if InStr(searchResult, join_raid)
				{
					updateLog("Clicking join raid")
					RandomClick(coordX + 7, coordY + 5, 0)
				}
